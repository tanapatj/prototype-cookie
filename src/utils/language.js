...
export const resolveCurrentLanguageCode = () =>  {
    ...
    if (autoDetect) {
        debug('ConsentManager [LANG]: autoDetect strategy: "' + autoDetect + '"');
...
export const loadTranslationData = async (desiredLanguageCode) => {
    ...
    debug('ConsentManager [LANG]: set language: "' + currentLanguageCode + '"');
...
export const handleRtlLanguage = () => {
    ...
    let cmMain = globalObj._dom._cmMain;

    if (rtlLanguages && cmMain) {
        if (!isArray(rtlLanguages))
            rtlLanguages = [rtlLanguages];

        elContains(rtlLanguages, globalObj._state._currentLanguageCode)
            ? addClass(cmMain, 'cm--rtl')
            : removeClass(cmMain, 'cm--rtl');
    }
};
