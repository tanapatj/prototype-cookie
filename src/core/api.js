...
export const show = (createModal) => {
     ...
     */
    setTimeout(() => {
        addClass(_dom._cmContainer, 'cm--anim');
        focus(globalObj._dom._cmDivTabindex);
    }, 100);

    debug('ConsentManager [TOGGLE]: show consentModal');
...
export const hide = () => {
     ...
     */
    setTimeout(() => removeClass(_dom._cmContainer, 'cm--anim'), 100);

    debug('ConsentManager [TOGGLE]: hide consentModal');
...
export const showPreferences = () => {
    ...
    debug('ConsentManager [TOGGLE]: show preferencesModal');

    fireEvent(globalObj._customEvents._onModalShow, PREFERENCES_MODAL_NAME);
};

/**
 * Discard unsaved preferences when closing the modal
 */
...
export const hidePreferences = () => {
    ...
    debug('ConsentManager [TOGGLE]: hide preferencesModal');
...
const retrieveState = () => {
    ...
    debug('ConsentManager [STATUS] valid consent:', !state._invalidConsent);
...
export const reset = (deleteCookie) => {
    //{{START: GUI}}
    const { _cmMain, _htmlDom } = globalObj._dom;
    //{{END: GUI}}
