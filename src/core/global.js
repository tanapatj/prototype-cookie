...
/**
 * @typedef {import('../../types')} ConsentManager
 *
 * @typedef {ConsentManager} Api
 * @typedef {ConsentManager.ConsentManagerConfig} UserConfig
 * @typedef {ConsentManager.Category} Category
 * @typedef {ConsentManager.Service} Service
 * @typedef {Object.<string, Service>} Services
 * @typedef {ConsentManager.AutoClear} AutoClear
 * @typedef {ConsentManager.GuiOptions} GuiOptions
 * @typedef {GuiOptions['consentModal']} GuiModalOption
 * @typedef {ConsentManager.ConsentManagerConfig['language']} Language
 * @typedef {ConsentManager.Translation} Translation
 * @typedef {ConsentManager.ConsentModalOptions} ConsentModalOptions
 * @typedef {ConsentManager.PreferencesModalOptions} PreferencesModalOptions
 * @typedef {ConsentManager.CookieTable} CookieTable
 * @typedef {ConsentManager.Section} Section
 * @typedef {ConsentManager.CookieValue} CookieValue
 * @typedef {ConsentManager.UserPreferences} UserPreferences
 */
...
export class GlobalState {
    constructor() {
         ...
         * @type {ConsentManager.ConsentManagerConfig}
         */
        this._config = {
            mode: OPT_IN_MODE,
            revision: 0,
        ...
        this._state = {
            ...
            */
            ...
            /**
            * @type {ConsentManager.AcceptType}
            */
            _acceptType: '',
        ...
        this._customEvents = {
            _onFirstConsent: 'cm:onFirstConsent',
            _onConsent: 'cm:onConsent',
            _onChange: 'cm:onChange',
            //{{START: GUI}}
            _onModalShow: 'cm:onModalShow',
            _onModalHide: 'cm:onModalHide',
            _onModalReady: 'cm:onModalReady'
            //{{END: GUI}}
        };
    }
}
